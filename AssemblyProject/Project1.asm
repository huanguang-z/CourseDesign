DATAS SEGMENT
   ...
   ERROR2 DB 'Reading error' , 07H, 0
	...
	menu  db 10,13,10,13,'           Which File do you want to select          '
      db 10,13
      db 10,13,'           1. FILE1'
      db 10,13,'           2. FILE2'
      db 10,13,'           3. FILE3'
      db 10,13
      db 10,13,'   please choose one of 1~3:','$'
...
freg:
      ...
      add bp, 2
      jmp freg
end_mus: ;根据存储在mus_freg和mus_time中的音频频率和时间信息,调用gensound过程来生成声音，并通过循环来遍历音乐数据
      ret		
music endp
...
NEXT0:  
    ...
    MOV SP,16
		MOV 	AH, 	9    			;09H功能号，显示DS：DL内字符串
      	MOV 	DX, 	OFFSET MESG1
      	INT 	21H
      	MOV 	AH, 	0AH   			;键盘输入字符串并显示，
		MOV 	DX, 	OFFSET USERNAME  ;用户输入字符串起始偏移位置
      	INT 	21H
      	MOV 	AH, 	2
      	MOV 	DL, 	0AH            	;换行字符
      	INT 	21H
...
File1:  MOV DX,OFFSET FNAME1
		JMP File
File2: MOV DX,OFFSET FNAME2
		JMP File
File3: MOV DX,OFFSET FNAME3
    	JMP File
...
DMESS1:
	...
	INT 21H ;显示一个字符
	JMP DMESS1
DMESS2:RET ;用栈中的数据，修改IP的内容，从而实现近转移，相当于 pop IP
DMESS ENDP
